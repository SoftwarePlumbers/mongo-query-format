...
describe('Range', () => {
    ...
    it('formats a simple query', () => { 
    	...
    	expect(query).to.equal('$and: [{ "a": "23" }, { $and: [{ "b": { $gte: 2 } }, { "b": { $lt: 4 } }] }]');
    });
    ...
    it('formats an or query', () => { 
    	...
    	expect(query).to.equal('$or: [{ $and: [{ "a": "23" }, { $and: [{ "b": { $gte: 2 } }, { "b": { $lt: 4 } }] }] }, { $and: [{ "a": "24" }, { $and: [{ "b": { $gte: 1 } }, { "b": { $lt: 16 } }] }] }]');
    });
    ...
    it('formats a subquery', () => { 
    	...
    	expect(query).to.equal('$and: [{ "a": "23" }, { b: { $elemMatch: { $and: [{ "c": 4 }, { "d": 2 }] } } }]');
    });
 });